#!/bin/bash
# RocketPi 一键编译脚本
# 使用方法: ./build_rocket_pi.sh [clean|flash|test]
...
TARGET="rocket_pi"
SAMPLE="zephyr/samples/ansilic/rocket_pi_shell_process"
...
case "$1" in
    ...
    flash)
        ...
        echo -e "${YELLOW}💡 打开串口: screen /dev/ttyACM0 115200${NC}"
        ;;
    test)
...
if [ $? -eq 0 ]; then
    ...
    if [ "$1" != "flash" ] && [ "$1" != "test" ]; then
        ...
        echo "  烧录: ./build_rocket_pi.sh flash"
        echo "  或手动: west flash -d $BUILD_DIR --runner openocd"
    fi
else
    echo -e "${RED}❌ 操作失败！${NC}"
    exit 1
